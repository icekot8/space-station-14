        ...
        public void RefreshTraits()
        {
            ...
            foreach (var (categoryId, traitId) in model)
            {
                ...
                foreach (var traitProto in traitId)
                {
                    ...
                    if (selector.Preference && trait.Slider)
                    {
                        TraitsList.AddChild(slider);

                        slider.OnValueChanged += (value1) =>
                        {
                            if (trait.Components?.OfType<StutteringAccentComponent>()?.FirstOrDefault() is StutteringAccentComponent accent)
                            {
                                accent.MatchRandomProb = value1.Value;
                                slider.Value = value1.Value;
                            }
                        };
                    }
                    ...
                    {
                        if (trait.Slider)
                            Profile = Profile?.WithTraitPreference(trait.ID, categoryId, preference, slider.Value);
                        else Profile = Profile?.WithTraitPreference(trait.ID, categoryId, preference, 0f);
                        SetDirty();
